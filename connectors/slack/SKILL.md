---
name: "slack"
description: "Read and write Slack: list channels, read history, post messages, list users. Trigger phrases: slack, slack channel, post to slack."
...
## Purpose
Read and write the user's Slack workspace: list channels, read recent history, post messages, list users. Use when the user mentions Slack or wants something sent to / found in a Slack channel.
...
- Note: Slack only grants `search:read` to user tokens, never bot tokens, so this skill has no search command. Requesting it as a bot scope makes Slack reject the whole install ("Invalid permissions requested").
